...
const Features = () => {
  ...
  useEffect(() => {
    ...
    const handleVoiceDownloadThumbnail = () => {
      if (generatedThumbnail || uploadedVideoId) {
  ...
  }, [uploadedVideoId, videoData, generatedThumbnail]);
        ...
        for (let attempt = 0; attempt < 4; attempt++) {
          try {
            ...
            if (updatedData.outputs?.processed_video) {
              const token = localStorage.getItem('token');
              const processedUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:5001/api'}/videos/${uploadedVideoId}/download?token=${token}&ts=${Date.now()}`;
  ...
  const handleProcessAudio = (overrides?: {
    forceEnableFillers?: boolean;
    forceCutVideo?: boolean;
    forceDetectRepeated?: boolean;
  }) => {
    if (!uploadedVideoId) {
    ...
    const forceEnableFillers = !!overrides?.forceEnableFillers;
    const effectiveRemoveFillers = forceEnableFillers ? true : removeFillers;
    const requestedCutVideo = forceEnableFillers
    ...
    console.log('[FRONTEND] removeFillers:', effectiveRemoveFillers);
    console.log('[FRONTEND] requestedCutVideo:', requestedCutVideo);
    console.log('[FRONTEND] removeFillersFromVideo:', effectiveRemoveFillersFromVideo);
    console.log('[FRONTEND] detectRepeatedWords:', effectiveDetectRepeated);
    ...
    const processingOptions = { 
      ...
      detect_and_remove_fillers: effectiveRemoveFillers,
      audio_enhancement_type: effectiveRemoveFillers ? fillerWordsLevel : 'medium',
    ...
    logToConsole(`Starting audio processing: Pause Threshold=${pauseThreshold}ms, Noise Reduction=${audioEnhancement}, Detect & Remove Fillers=${effectiveRemoveFillers}, Cut Video Segments=${effectiveRemoveFillersFromVideo}, Level=${fillerWordsLevel}`);
...
Automatisch generiert von SnipX AI`
  ...
  const handleGenerateThumbnail = async () => {
    ...
    try {
      const waitForThumbnailAvailability = async (videoId: string, timeoutMs = 180000, intervalMs = 1500) => {
        const attempts = Math.max(1, Math.ceil(timeoutMs / intervalMs));

        for (let attempt = 0; attempt < attempts; attempt++) {
          const latest = await ApiService.getVideoStatus(videoId);
          setVideoData(latest);

          const hasThumbnail = Boolean(latest?.outputs?.thumbnail) || Boolean(latest?.outputs?.thumbnails?.length);
          if (hasThumbnail) {
            return latest;
          }

          if (latest?.status === 'failed') {
            throw new Error(latest?.error || 'Thumbnail generation failed on server');
          }

          await new Promise(resolve => setTimeout(resolve, intervalMs));
        }

        throw new Error('Thumbnail generation is taking longer than expected. Please wait a bit and try download again.');
      };
      ...
      // Wait until backend actually stores thumbnail outputs (processVideo request returns before processing is done).
      await waitForThumbnailAvailability(uploadedVideoId);
      
      if (isAuthenticated && uploadedVideoId) {
        // Get the real thumbnail URL from backend with cache busting
        const timestamp = Date.now();
        const baseUrl = ApiService.getVideoThumbnailUrl(uploadedVideoId);
        const thumbnailUrl = `${baseUrl}${baseUrl.includes('?') ? '&' : '?'}t=${timestamp}`;
  ...
  const handleDownloadThumbnail = async () => {
    ...
    try {
      ...
      // Use dedicated backend download endpoint for more reliable auth + binary response handling.
      if (uploadedVideoId) {
        const hasThumbnail = Boolean(videoData?.outputs?.thumbnail) || Boolean(videoData?.outputs?.thumbnails?.length);

        if (!hasThumbnail) {
          logToConsole('Thumbnail is still generating on server, waiting before download...', 'info');

          const maxAttempts = 60;
          for (let i = 0; i < maxAttempts; i++) {
            const latest = await ApiService.getVideoStatus(uploadedVideoId);
            setVideoData(latest);

            const ready = Boolean(latest?.outputs?.thumbnail) || Boolean(latest?.outputs?.thumbnails?.length);
            if (ready) {
              break;
            }
            ...
            if (i === maxAttempts - 1) {
              throw new Error('Thumbnail is still generating. Please wait and try again in a few moments.');
            }
        ...
        blob = await ApiService.downloadThumbnail(uploadedVideoId);
                          ...
                          try {
                            ...
                            if (showProcessedVideo) {
                              ...
                              if (selectedFile) {
                                ...
                                const response = await fetch(`${apiUrl}/videos/${uploadedVideoId}/download/original?ts=${Date.now()}`, {
                                  cache: 'no-store',
                                  headers: {
                                    'ngrok-skip-browser-warning': 'true',
                                    'Authorization': `Bearer ${token}`
                              ...
                              const response = await fetch(`${apiUrl}/videos/${uploadedVideoId}/download?ts=${Date.now()}`, {
                                cache: 'no-store',
                                headers: {
                                  'ngrok-skip-browser-warning': 'true',
                                  'Authorization': `Bearer ${token}`
