...
class AudioEnhancer:
    ...
    def _match_filler(cls, word: str, single_word_fillers: set[str]):
        """Return a canonical filler match string if token should be treated as filler, else None."""
        # Direct match against normalized filler vocabulary
        normalized = cls._normalize_token(word)
        if not normalized:
            return None

        # Build normalized lookup once per call-site by passing pre-normalized set
        ...
        # Handle common hyphenated fillers when punctuation is stripped
        if normalized in {"mmhmm", "uhhuh", "mhm"}:
            return normalized
    ...
    def enhance_audio(self, audio_path, options):
        ...
        try:
            ...
            forced_filler_segments = options.get('forced_filler_segments', []) or []
            ...
            if detect_and_remove_fillers or detect_only_for_video_cutting:
                if detect_only_for_video_cutting:
                    ...
                    _ = self._remove_filler_words_with_whisper(
                        ...
                        enable_gap_filler_detection=bool(options.get('enable_gap_filler_detection', False)),
                        forced_filler_segments=forced_filler_segments,
                    )
    ...
    def _remove_filler_words_with_whisper(self, audio, audio_path, enhancement_type, detect_repeated=True, custom_fillers=None, enable_gap_filler_detection=False, forced_filler_segments=None):
        ...
        try:
            ...
            # If caller provides transcript-aligned segments, use those as source of truth.
            if forced_filler_segments:
                normalized_forced = []
                for seg in forced_filler_segments:
                    try:
                        if isinstance(seg, dict):
                            start_ms = int(float(seg.get('start_ms', seg.get('start', 0))))
                            end_ms = int(float(seg.get('end_ms', seg.get('end', 0))))
                        else:
                            start_ms = int(float(seg[0]))
                            end_ms = int(float(seg[1]))
                    except Exception:
                        continue
                    if end_ms > start_ms:
                        normalized_forced.append((start_ms, end_ms))

                normalized_forced.sort(key=lambda x: x[0])
                self._detected_filler_segments = normalized_forced
                self._filler_words_removed_count = 0

                if not normalized_forced:
                    print("[FILLER] Forced segment list was empty after normalization")
                    return audio

                print(f"[FILLER] Using {len(normalized_forced)} forced transcript-aligned segments")
                filler_segments = normalized_forced
            else:
            # Use custom filler words if provided, otherwise use predefined lists
                ...
                filler_segments = self._detect_fillers_with_whisper(
                    audio_path,
                    target_fillers,
    ...
    def _detect_fillers_with_whisper(self, audio_path, target_fillers, detect_repeated=True, enable_gap_filler_detection=False):
        ...
        try:
            ...
            if not WHISPER_TS_AVAILABLE:
                print("[WHISPER] whisper-timestamped not available; cannot do reliable word-level filler detection")
                return []
            
            print(f"[WHISPER] Transcribing audio for filler and repeated word detection...")
            
            # Keep prompt neutral to reduce filler hallucinations on Hinglish/noisy audio.
            filler_prompt = (
            ...
            for segment in result.get('segments', []):
                ...
                for word_info in words:
                    raw_word = (word_info.get('word') or word_info.get('text') or '').strip()

                    # If a disfluency marker appears, cut by timestamp but do not invent text like "uhm".
                    if raw_word == "[*]":
                        start_ms = int(float(word_info.get('start', 0)) * 1000)
                        end_ms = int(float(word_info.get('end', 0)) * 1000)
                        if end_ms > start_ms:
                            filler_segments.append((start_ms, end_ms, "disfluency"))
                            print(f"[WHISPER] Found disfluency marker at {start_ms}ms - {end_ms}ms")
                        continue

                    word_clean = self._normalize_token(raw_word)
                    if word_clean:
                        all_words.append({
                            'text': word_clean,
                            'original': raw_word,
            ...
            # Normalize filler vocabulary to match punctuation-stripped tokens.
            raw_single_word_fillers = [f for f in target_fillers if ' ' not in f]
            single_word_fillers = set(self._normalize_token(f) for f in raw_single_word_fillers)

            for word_info in all_words:
                word_clean = word_info['text']

                matched = self._match_filler(word_clean, single_word_fillers)
                if matched:
    ...
    def generate_transcript_with_fillers(self, audio_path, enhancement_type='medium', detect_repeated=True):
        ...
        try:
            ...
            # IMPORTANT: Do NOT prime with a filler-heavy prompt.
            # That can cause hallucinated "uh/um" loops on noisy/low-speech audio.
            transcript_prompt = (
                "Transcribe the audio verbatim. Keep hesitation sounds like um/uh when spoken. "
            ...
            def _extract_all_words(result_obj):
                extracted = []
                for seg in result_obj.get('segments', []):
                    for word_info in seg.get('words', []):
                        raw_word = word_info.get('word', '') or word_info.get('text', '')
                        raw_word_stripped = str(raw_word).strip()

                        # Ignore disfluency marker token instead of injecting synthetic filler text.
                        if raw_word_stripped == "[*]":
                            continue

                        token_norm = self._normalize_token(raw_word_stripped)
                        if token_norm:
                            extracted.append({
                                'text': token_norm,
                                'original': raw_word_stripped,
                                'start': word_info.get('start', 0),
                                'end': word_info.get('end', 0),
            ...
            # Sanity check: if Whisper output is overwhelmingly fillers, retry with safer settings
            if len(all_words) >= 30:
                raw_single_word_fillers = [f for f in target_fillers if ' ' not in f]
                filler_vocab = set(self._normalize_token(f) for f in raw_single_word_fillers)
                filler_like = 0
                for wi in all_words:
                    if self._match_filler(wi.get('text', ''), filler_vocab) is not None:
                        filler_like += 1
                filler_ratio = filler_like / max(len(all_words), 1)

                if filler_ratio >= 0.85:
                    print(f"[TRANSCRIPT] ⚠️ Filler-dominant transcript detected ({filler_ratio:.2%}). Retrying with stricter no-speech + no disfluency...")
                    try:
                        retry = _run_ts(
                            vad=False,
                            detect_disfluencies=False,
                        ...
                        all_words_retry = _extract_all_words(retry)
                        if all_words_retry and len(all_words_retry) >= 10:
                            result = retry
                            all_words = all_words_retry
                            print(f"[TRANSCRIPT] Retry accepted: {len(all_words)} words")
                    except Exception as retry_err:
                        print(f"[TRANSCRIPT] Retry failed (non-fatal): {retry_err}")
            
            # Detect single-word fillers with robust normalization
            raw_single_word_fillers = [f for f in target_fillers if ' ' not in f]
            single_word_fillers = set(self._normalize_token(f) for f in raw_single_word_fillers)
            ...
            for i, word_info in enumerate(all_words):
                ...
                # Check 1: Filler match
                matched = self._match_filler(word_clean, single_word_fillers)
                if matched is not None:
                    is_filler = True
                    detected_fillers.append(word_clean)
                    print(f"[TRANSCRIPT] Detected filler (match): '{word_clean}' at {word_info['start']:.2f}s")
            ...
            # Do not synthesize transcript fillers from gaps.
            # Transcript must stay aligned to actual spoken/timestamped tokens.
            ...
            transcript_data = {
                ...
                # Backwards/endpoint compatibility
...
class VideoService:
    ...
    def save_video(self, file, user_id):
        ...
        if not filename:
            filename = f"upload_{uuid4().hex[:8]}.mp4"
        ...
        stored_filename = f"{name_root}_{unique_suffix}_{uuid4().hex[:8]}{ext}"
    ...
    def process_video(self, video_id, options):
        ...
        def _is_non_default_percent(value):
            """Return True only when a 0-200 slider-like value differs from default 100."""
            if value is None:
                return False
            if isinstance(value, str) and not value.strip():
                return False
            try:
                return abs(float(value) - 100.0) > 1e-6
            except Exception:
                return bool(value)

        stabilization_value = str(options.get('stabilization', 'none')).strip().lower()
        stabilization_enabled = stabilization_value not in {'', 'none', 'off', 'false', '0'}
        has_video_enhancements = any([
            stabilization_enabled,
            bool(options.get('ai_color_enhancement', False)),
            _is_non_default_percent(options.get('brightness')),
            _is_non_default_percent(options.get('contrast')),
            _is_non_default_percent(options.get('saturation')),
        ])
        ...
        try:
            ...
            transcript_words = (video.transcript or {}).get('words', []) if isinstance(video.transcript, dict) else []
            transcript_has_fillers = any(bool((w or {}).get('is_filler', False)) for w in transcript_words if isinstance(w, dict))

            should_cut_filler_segments = bool(
            ...
            # Safety net: when transcript already has highlighted fillers, ensure process-stage cut runs.
            if (not should_cut_filler_segments) and bool(options.get('enhance_audio', False)) and transcript_has_fillers:
                should_cut_filler_segments = True
                print("[VIDEO SERVICE] Auto-enabling filler cut because transcript contains highlighted fillers")
            ...
            if should_cut_filler_segments:
                ...
                # 1. Yeh video ko cut karega
                applied_segments = self._remove_filler_segments_from_video(video, options, filler_segments)
                
                # ✅ 2. YEH NAYA CODE HAI: Video cut hone ke foran baad transcript ki timings theek karega
                if applied_segments:
                    self._sync_transcript_timestamps(video, applied_segments)
    ...
    def _apply_video_enhancements(self, video, options):
        ...
        try:
            ...
            print(f"[VIDEO ENHANCE] Source video for enhancement: {source_video_path}")
            ...
            if use_ai_enhancement:
                ...
                analysis = self.color_enhancer.analyze_video_colors(source_video_path)
                if analysis:
            ...
            video_tag = str(getattr(video, '_id', 'video'))[:8]
            output_path = f"{os.path.splitext(source_video_path)[0]}_{video_tag}_enhanced.mp4"
    ...
    def _enhance_audio(self, video, options):
        ...
        try:
            ...
            video_tag = str(getattr(video, '_id', 'video'))[:8]
            audio_path = f"{os.path.splitext(video.filepath)[0]}_{video_tag}_temp_audio.wav"
            ...
            # Keep transcript labels from generate_transcript_with_fillers as source of truth.
            # Re-marking by broad time overlap can mislabel repeated/normal words as fillers.
            
            # Save enhanced audio temporarily
            enhanced_audio_path = f"{os.path.splitext(video.filepath)[0]}_{video_tag}_enhanced_audio.wav"
            print(f"[VIDEO SERVICE] Saving enhanced audio to: {enhanced_audio_path}")
            if len(enhanced_audio) < 200:
                # Guardrail: avoid writing near-empty/corrupt audio that crashes MoviePy reader.
                print(f"[VIDEO SERVICE] ⚠️ Enhanced audio too short ({len(enhanced_audio)}ms), falling back to original extracted audio")
                enhanced_audio = AudioSegment.from_file(audio_path)
            ...
            audio_duration = float(enhanced_audio_clip.duration or 0.0)

            # If enhanced track is invalid, keep original audio instead of crashing processing.
            if audio_duration <= 0.05:
                print(f"[VIDEO SERVICE] ⚠️ Enhanced audio duration invalid ({audio_duration:.3f}s), using original video audio")
                enhanced_audio_clip.close()
                if clip.audio is None:
                    raise ValueError("Video has no original audio track to fall back to")
                enhanced_audio_clip = clip.audio
                audio_duration = float(getattr(enhanced_audio_clip, 'duration', video_duration) or video_duration)

            safe_duration = min(video_duration, audio_duration) if video_duration > 0 else audio_duration
            if safe_duration <= 0.05:
                raise ValueError(
                    f"Invalid export duration (video={video_duration:.3f}s, audio={audio_duration:.3f}s)"
                )

            if abs(video_duration - audio_duration) > 0.05:
                print(
                    f"[VIDEO SERVICE] Duration mismatch before mux: "
                    f"video={video_duration:.3f}s, audio={audio_duration:.3f}s; trimming to {safe_duration:.3f}s"
                )

            video_for_mux = clip.subclip(0, safe_duration) if video_duration - safe_duration > 0.05 else clip
            audio_for_mux = (
                enhanced_audio_clip.subclip(0, safe_duration)
                if audio_duration - safe_duration > 0.05 else enhanced_audio_clip
            )
            enhanced_clip = video_for_mux.set_audio(audio_for_mux)
    ...
    def _normalize_cut_segments_ms(self, segments, max_duration_ms=None, merge_gap_ms=80):
        """Normalize, clamp, sort, and merge millisecond segments for stable cutting."""
        normalized = []
        for seg in segments or []:
            ...
            if max_duration_ms is not None:
                start_ms = max(0, min(start_ms, int(max_duration_ms)))
                end_ms = max(0, min(end_ms, int(max_duration_ms)))
        ...
        for cur_start, cur_end in normalized[1:]:
            last_start, last_end = merged[-1]
            if cur_start <= last_end + int(merge_gap_ms):
    ...
    def _build_filler_segments_from_transcript(self, transcript, include_repeated=False, pre_pad_ms=0, post_pad_ms=75):
        """Create cut segments directly from transcript word flags to keep highlight/cut alignment exact."""
        words = (transcript or {}).get('words', []) or []
        raw_segments = []
        base_pre = max(0, int(pre_pad_ms or 0))
        base_post = max(0, int(post_pad_ms or 0))
        core_fillers = {
            'um', 'uh', 'uhm', 'umm', 'erm', 'er', 'ah', 'hmm', 'hm', 'mm', 'mmm', 'mhm', 'huh'
        }
        ...
        for word in words:
            ...
            # Strict mode: preserve non-filler words by keeping left cut almost zero.
            dyn_pre_pad = int(min(base_pre, 6 if is_core_filler else 3))
            dyn_post_pad = int(max(base_post, 220 if is_core_filler else 55))
            min_cut_ms = 220 if is_core_filler else 70

            # Extra right-tail for short core fillers to avoid audible residue in final voice track.
            core_tail_boost_ms = 80 if (is_core_filler and word_norm in {'uh', 'um', 'uhm', 'umm', 'hm', 'hmm'}) else 0

            start_ms = max(0, int(round(start_s * 1000)) - dyn_pre_pad)
            end_ms = max(start_ms + min_cut_ms, int(round(end_s * 1000)) + dyn_post_pad + core_tail_boost_ms)
            raw_segments.append((start_ms, end_ms))

        # Fallback: some transcripts keep timings in filler_words while word flags are incomplete.
        if not raw_segments:
            for fw in (transcript or {}).get('filler_words', []) or []:
                if not isinstance(fw, dict):
                    continue
                try:
                    start_s = float(fw.get('start', 0.0))
                    end_s = float(fw.get('end', start_s))
                except Exception:
                    continue
                if end_s <= start_s:
                    continue
                ...
                end_ms = max(start_ms + 180, int(round(end_s * 1000)) + int(max(base_post, 160)))
                raw_segments.append((start_ms, end_ms))

        # Keep merge gap tight for word-accurate cuts.
        return self._normalize_cut_segments_ms(raw_segments, merge_gap_ms=8)

    def _refine_cut_segments_with_audio_boundaries(self, media_path, segments_ms, max_left_expand_ms=10, max_right_expand_ms=150, step_ms=10, min_cut_ms=110):
        """Expand segment edges to nearby low-energy boundaries to remove complete filler utterances naturally."""
        try:
            normalized = self._normalize_cut_segments_ms(segments_ms, merge_gap_ms=12)
            if not normalized:
                return []
            ...
            overall_db = audio.dBFS if audio.dBFS != float('-inf') else -45.0
            # Dynamic threshold tuned for spoken filler boundary detection.
            silence_thresh = max(-50.0, min(-30.0, overall_db - 12.0))
            refined = []

            for start_ms, end_ms in normalized:
                s = max(0, min(int(start_ms), total_ms))
                e = max(0, min(int(end_ms), total_ms))
                if e <= s:
                    continue

                # Expand left while speech energy is present.
                expanded = 0
                while s - step_ms >= 0 and expanded < max_left_expand_ms:
                    probe = audio[max(0, s - step_ms):s]
                    if len(probe) == 0:
                        break
                    if probe.dBFS == float('-inf') or probe.dBFS < silence_thresh:
                        break
                    s -= step_ms
                    expanded += step_ms

                # Expand right while speech energy is present.
                expanded = 0
                while e + step_ms <= total_ms and expanded < max_right_expand_ms:
                    probe = audio[e:min(total_ms, e + step_ms)]
                    if len(probe) == 0:
                        break
                    if probe.dBFS == float('-inf') or probe.dBFS < silence_thresh:
                        break
                ...
                # Enforce minimum cut duration for audible fillers (um/uh often under-timestamped).
                if e - s < min_cut_ms:
                    missing = min_cut_ms - (e - s)
                    # Favor right extension to avoid eating preceding lexical words.
                    left_extra = int(missing * 0.05)
                    right_extra = missing - left_extra
                    s = max(0, s - left_extra)
                    e = min(total_ms, e + right_extra)

                if e > s:
                    refined.append((s, e))

            final_segments = self._normalize_cut_segments_ms(refined, max_duration_ms=total_ms, merge_gap_ms=12)
            print(f"[VIDEO FILLER REMOVAL] Audio-boundary refinement: {len(normalized)} -> {len(final_segments)} segments")
            return final_segments
        except Exception as e:
            print(f"[VIDEO FILLER REMOVAL] Boundary refinement skipped due to error: {e}")
            return self._normalize_cut_segments_ms(segments_ms, merge_gap_ms=12)
    ...
    def _remove_filler_segments_from_video(self, video, options, filler_segments=None):
        ...
        """
        try:
            # Keep enable logic aligned with process-stage trigger.
            should_cut_filler_segments = bool(
                options.get('cut_filler_segments', options.get('remove_filler_words', False))
                or options.get('detect_and_remove_fillers', options.get('remove_fillers', False))
            )
            ...
            # Use provided filler segments from pipeline.
            # If missing, rebuild strictly from transcript highlighted fillers as fallback.
            if not filler_segments:
                fallback_transcript = video.transcript if isinstance(video.transcript, dict) else {}
                filler_segments = self._build_filler_segments_from_transcript(
                    fallback_transcript,
                    include_repeated=False,
                    pre_pad_ms=int(options.get('filler_cut_pre_pad_ms', 0) or 0),
                    post_pad_ms=int(options.get('filler_cut_post_pad_ms', 55) or 55),
                )
                if filler_segments:
                    print(f"[VIDEO FILLER REMOVAL] Rebuilt {len(filler_segments)} filler segments from transcript fallback")
                else:
                    print("[VIDEO FILLER REMOVAL] No filler segments provided/rebuilt, skipping video cutting")
                    return []

            print(f"[VIDEO FILLER REMOVAL] Using {len(filler_segments)} pre-detected filler segments")
            
            # For exact transcript alignment, cut against original timeline by default.
            # Optional override keeps previous behavior when explicitly requested.
            prefer_enhanced_source = bool(options.get('use_enhanced_source_for_filler_cut', False))
            source_path = video.outputs.get("processed_video", video.filepath) if prefer_enhanced_source else video.filepath
            if source_path != video.filepath:
                print(f"[VIDEO FILLER REMOVAL] Using enhanced source by option: {source_path}")
            else:
                print(f"[VIDEO FILLER REMOVAL] Using original source for exact timing: {source_path}")
            clip = VideoFileClip(source_path)
            
            # Create list of segments to KEEP (inverse of segments to remove)
            video_duration_ms = int(clip.duration * 1000)
            filler_segments = self._normalize_cut_segments_ms(filler_segments, max_duration_ms=video_duration_ms, merge_gap_ms=8)
            if not filler_segments:
                print("[VIDEO FILLER REMOVAL] All filler segments were invalid after normalization")
                clip.close()
                return []
            ...
            video_tag = str(getattr(video, '_id', 'video'))[:8]
            output_path = f"{os.path.splitext(video.filepath)[0]}_{video_tag}_cleaned.mp4"
    ...
    def _cut_video_segments_ffmpeg(self, input_path, keep_segments, output_path, smooth_transitions=True):
        """
        Cut video into segments and concatenate using a single FFmpeg filter graph.
        This keeps cut timing aligned to transcript timestamps more accurately than
        per-segment extraction + concat workflows.
        ...
        try:
            ...
            for seg in keep_segments or []:
                ...
                if end_sec > start_sec:
                    normalized_keep.append((start_sec, end_sec))

            if not normalized_keep:
                raise Exception("No valid keep segments provided")

            filter_parts = []
            concat_inputs = []
            for i, (start_sec, end_sec) in enumerate(normalized_keep):
                print(
                    f"[FFMPEG VIDEO CUT] Keep segment {i + 1}/{len(normalized_keep)}: "
                    f"{start_sec:.3f}s - {end_sec:.3f}s"
                )
                filter_parts.append(
                    f"[0:v]trim=start={start_sec:.6f}:end={end_sec:.6f},setpts=PTS-STARTPTS[v{i}]"
                )
                filter_parts.append(
                    f"[0:a]atrim=start={start_sec:.6f}:end={end_sec:.6f},asetpts=PTS-STARTPTS[a{i}]"
                )
                concat_inputs.append(f"[v{i}][a{i}]")

            filter_parts.append(
                f"{''.join(concat_inputs)}concat=n={len(normalized_keep)}:v=1:a=1[vout][aout]"
            )
            ...
            print(f"[FFMPEG VIDEO CUT] Running single-pass trim+concat pipeline...")
            result = subprocess.run(concat_cmd, capture_output=True, text=True)
            if result.returncode != 0:
                print(f"[FFMPEG VIDEO CUT] Primary concat failed, trying libx264 fallback: {result.stderr[-300:]}")
                fallback_cmd = [
                    'ffmpeg',
                    '-i', input_path,
                    '-filter_complex', filter_complex,
                ...
                if result.returncode != 0:
                    raise Exception(f"FFmpeg trim+concat failed: {result.stderr[-300:]}")
    ...
    def _generate_subtitles(self, video, options):
        ...
        try:
            ...
            audio_path = f"{os.path.splitext(source_video_path)[0]}_audio.wav"
...
{{{{
  ...
  "key_points": ["Point 1", "Point 2", "Point 3"]
    ...
    def export_video_with_edits(self, video_id, trim_start=0, trim_end=100,
        ...
        """Export video with edits. Supports single trim or multiple trim segments merged into one output."""
        import subprocess
        import tempfile
        ...
        print(f"[EXPORT] Single trim fallback: {trim_start}% - {trim_end}%")
        print(f"[EXPORT] Multi trim segments: {trim_segments}")
        ...
        # Build normalized list of segments in percentages
        if isinstance(trim_segments, list) and trim_segments:
            for segment in trim_segments:
                if not isinstance(segment, dict):
                    continue
                start_pct = _clamp_percent(segment.get('start', 0))
                end_pct = _clamp_percent(segment.get('end', 100))
                if end_pct - start_pct >= 1.0:
                    normalized_segments.append({'start': start_pct, 'end': end_pct})

        # Backward compatibility: use single-range trim if no valid segment list was sent.
        if not normalized_segments:
            single_start = _clamp_percent(trim_start)
            single_end = _clamp_percent(trim_end)
            if single_end - single_start >= 1.0 and not (single_start == 0 and single_end == 100):
                normalized_segments = [{'start': single_start, 'end': single_end}]

        # Sort and merge overlapping/adjacent segments in percentage space.
        ...
        print(f"[EXPORT] Duration={duration:.2f}s | Final segments={normalized_segments if normalized_segments else 'FULL VIDEO'}")

        # Step 1: If segment trim requested, produce a temporary trimmed/merged input file.
        if normalized_segments:
            keep_segments = [
                ((segment['start'] / 100.0) * duration, (segment['end'] / 100.0) * duration)
                for segment in normalized_segments
            ]

            temp_trimmed = os.path.join(
                self.upload_folder,
                f"temp_trim_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.mp4"
            )

            if len(keep_segments) == 1:
                start_sec, end_sec = keep_segments[0]
                trim_dur = max(0.1, end_sec - start_sec)
                trim_cmd = [
                    ffmpeg_path,
                    '-y',
                    '-ss', str(start_sec),
                ...
                print(f"[EXPORT] Running single-trim command: {' '.join(trim_cmd)}")
                trim_result = subprocess.run(trim_cmd, capture_output=True, text=True, timeout=300)
                if trim_result.returncode != 0:
                    raise Exception(f"Single-trim FFmpeg failed: {trim_result.stderr[-500:]}")
            else:
                print(f"[EXPORT] Running multi-segment trim+merge for {len(keep_segments)} segments")
                self._cut_video_segments_ffmpeg(video.filepath, keep_segments, temp_trimmed, smooth_transitions=True)

            working_input = temp_trimmed
            temp_files.append(temp_trimmed)

        # Step 2: Apply overlays/audio settings on the working input.
        ...
        cmd = [ffmpeg_path, '-y', '-i', working_input]
        video_filters = []
        ...
        if text_overlay and text_overlay.strip():
            ...
            color = str(text_color).lstrip('#') or 'ffffff'

            temp_text_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.txt', encoding='utf-8')
            temp_text_file.write(text_overlay)
            temp_text_file.close()
            temp_files.append(temp_text_file.name)
            text_file_path = temp_text_file.name.replace('\\', '/')

            text_filter = (
                f"drawtext=fontfile=C\\:/Windows/Fonts/arialbd.ttf:"
                f"textfile='{text_file_path}':{pos}:fontsize={text_size}:"
                f"fontcolor=0x{color}:borderw=3:bordercolor=black"
            )
        ...
        if mute_original:
            ...
            volume = max(0.0, float(video_volume) / 100.0)
        ...
        print(f"[EXPORT] Running final FFmpeg command: {' '.join(cmd)}")
        ...
        try:
            ...
            if normalized_segments:
                metadata_update["metadata.trim_segments"] = normalized_segments

            self.videos.update_one({"_id": ObjectId(video_id)}, {"$set": metadata_update})
            ...
            for temp_path in temp_files:
                try:
                    ...
                    pass
    def _sync_transcript_timestamps(self, video, filler_segments):
        """Sync transcript with cleaned timeline by removing cut words and shifting kept words."""
        try:
            transcript = video.transcript if isinstance(video.transcript, dict) else None
            if not transcript or 'words' not in transcript:
                return

            normalized_cuts = self._normalize_cut_segments_ms(filler_segments, merge_gap_ms=8)
            if not normalized_cuts:
                return

            print(f"[SYNC] Updating transcript for {len(normalized_cuts)} cut segments...")
            cuts_in_seconds = [(start / 1000.0, end / 1000.0) for start, end in normalized_cuts]

            def _shift_time(timestamp_s):
                shift = 0.0
                for cut_start, cut_end in cuts_in_seconds:
                    if cut_end <= timestamp_s:
                        shift += (cut_end - cut_start)
                    elif cut_start < timestamp_s < cut_end:
                        shift += (timestamp_s - cut_start)
                return max(0.0, timestamp_s - shift)
            ...
            transcript['words'] = adjusted_words
            transcript['total_words'] = len(adjusted_words)
            transcript['duration'] = adjusted_words[-1]['end'] if adjusted_words else 0
            transcript['filler_count'] = sum(1 for w in adjusted_words if w.get('is_filler'))
            transcript['repeated_count'] = sum(1 for w in adjusted_words if w.get('is_repeated'))
            ...
            transcript['text'] = ' '.join(str(w.get('text', '')).strip() for w in adjusted_words if str(w.get('text', '')).strip())
            ...
            print(f"[SYNC] Transcript synced. Removed {removed_words} cut words, kept {len(adjusted_words)} words.")
        except Exception as e:
            print(f"[SYNC] Error syncing transcript: {e}")
