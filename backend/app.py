...
def _set_no_cache_headers(response):
    """Ensure browser always fetches latest generated media files."""
...
def export_video(user_id, video_id):
    try:
        ...
        logger.info(f"Exporting video {video_id} with edits: trim={trim_start}-{trim_end}%, segments={trim_segments}, text='{text_overlay}'")
...
def merge_videos(user_id):
    """Merge multiple user videos into a single MP4 output."""
    clips = []
    final_clip = None
    ...
    try:
        ...
        for video in videos:
            source_path = video.outputs.get('exported_video') or video.outputs.get('processed_video') or video.filepath
            if not os.path.isabs(source_path):
                source_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), source_path)

            if not os.path.exists(source_path):
                return jsonify({'error': 'Source file missing for one of the selected videos'}), 404

            clips.append(VideoFileClip(source_path))
        ...
        final_clip.write_videofile(
            ...
            codec='libx264',
            audio_codec='aac',
            temp_audiofile=f'temp-audio-{timestamp}.m4a',
            remove_temp=True,
            logger=None
        )
...
@require_auth
def get_dubbing_languages(user_id, video_id):
    """Return supported language list for AI dubbing UI."""
    try:
        video = video_service.get_video(video_id)
        if not video:
            return jsonify({'error': 'Video not found'}), 404
        ...
        logger.error(f"Get dubbing languages error: {str(e)}")
        return jsonify({'error': 'Internal server error'}), 500

@app.route('/api/videos/<video_id>/dub', methods=['POST'])
@require_auth
def dub_video(user_id, video_id):
    """Create a translated dubbed video variant and return a new video id."""
    source_clip = None
    output_clip = None
    tts_audio_clip = None
    ...
    try:
        ...
        source_language = data.get('sourceLanguage') or data.get('source_language') or 'auto'
        mix_original = bool(data.get('mixOriginal', data.get('mix_original', False)))

        if not target_language:
            return jsonify({'error': 'targetLanguage is required'}), 400

        if target_language not in SUPPORTED_DUBBING_LANGUAGES:
            return jsonify({
                'error': 'Unsupported target language',
                'supported_languages': SUPPORTED_DUBBING_LANGUAGES
            }), 400
        ...
        timestamp = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
        base_name = os.path.splitext(video.filename)[0]
        output_filename = f"{base_name}_dubbed_{target_language}_{timestamp}.mp4"
        ...
        transcript_text = ''
        if isinstance(video.transcript, dict):
            transcript_text = (video.transcript.get('text') or '').strip()

        # Generate transcript on-demand if it is missing.
        if not transcript_text:
            try:
                video_service._auto_transcribe_video(video_id, source_path)
                refreshed_video = video_service.get_video(video_id)
                if refreshed_video and isinstance(refreshed_video.transcript, dict):
                    transcript_text = (refreshed_video.transcript.get('text') or '').strip()
            except Exception as transcribe_error:
                logger.error(f"Dub transcript generation failed: {str(transcribe_error)}")

        if not transcript_text:
            return jsonify({'error': 'Unable to generate transcript for dubbing'}), 500

        try:
            from deep_translator import GoogleTranslator
        except ImportError:
            return jsonify({'error': 'deep-translator dependency missing on backend'}), 500

        try:
            from gtts import gTTS
        except ImportError:
            return jsonify({'error': 'gTTS dependency missing on backend'}), 500

        translation_source = 'auto' if source_language == 'auto' else source_language
        try:
            translated_text = GoogleTranslator(source=translation_source, target=target_language).translate(transcript_text)
        except Exception as translate_error:
            logger.error(f"Dub translation failed: {str(translate_error)}")
            return jsonify({'error': f'Translation failed: {str(translate_error)}'}), 500

        if not translated_text or not str(translated_text).strip():
            return jsonify({'error': 'Translation produced empty text'}), 500

        temp_tts_path = os.path.join(upload_folder, f"tts_{timestamp}_{target_language}.mp3")
        try:
            tts_engine = gTTS(text=translated_text, lang=target_language)
            tts_engine.save(temp_tts_path)
        except Exception as tts_error:
            logger.error(f"Dub TTS generation failed: {str(tts_error)}")
            return jsonify({'error': f'TTS generation failed: {str(tts_error)}'}), 500

        tts_audio_clip = AudioFileClip(temp_tts_path)

        if tts_audio_clip.duration < source_clip.duration:
            tts_audio_clip = audio_loop(tts_audio_clip, duration=source_clip.duration)
        else:
            tts_audio_clip = tts_audio_clip.subclip(0, source_clip.duration)

        if mix_original and source_clip.audio is not None:
            final_audio_clip = CompositeAudioClip([
                source_clip.audio.volumex(0.2),
                tts_audio_clip.volumex(1.0)
            ])
        else:
            final_audio_clip = tts_audio_clip

        output_clip = source_clip.set_audio(final_audio_clip)
        output_clip.write_videofile(
            output_path,
            codec='libx264',
            audio_codec='aac',
            temp_audiofile=f'temp-dub-audio-{timestamp}.m4a',
            remove_temp=True,
            logger=None
        )

        dubbed_duration = source_clip.duration
        ...
        dubbed_video_doc = {
            ...
            'error': None,
            'metadata': {
                'duration': dubbed_duration,
                'dubbed_from': video_id,
                'target_language': target_language,
                'source_language': source_language,
                'mix_original': mix_original,
                'dubbing_engine': 'whisper+deep-translator+gTTS',
                'format': 'mp4',
                'resolution': None,
                'fps': None
            },
        ...
        result = video_service.videos.insert_one(dubbed_video_doc)
        dubbed_video_id = str(result.inserted_id)

        return jsonify({
            'message': 'Dubbing completed successfully',
            'video_id': dubbed_video_id,
            'filename': output_filename,
            'targetLanguage': target_language,
            'targetLanguageName': SUPPORTED_DUBBING_LANGUAGES[target_language],
        ...
        if output_clip is not None and output_clip is not source_clip:
            try:
                output_clip.close()
            except Exception:
                pass
        if source_clip is not None:
            try:
                ...
                pass
        if final_audio_clip is not None:
            try:
                final_audio_clip.close()
            except Exception:
                pass
        if tts_audio_clip is not None:
            try:
...
def detect_filler_words(user_id, video_id):
    ...
    try:
        ...
        # Backwards compatibility: older transcripts only store per-word flags in transcript['words']
        if filler_words is None or repeated_words is None:
            filler_words = []
            repeated_words = []
            for w in transcript_data.get('words', []) or []:
                ...
                if w.get('is_filler'):
                    filler_words.append({'word': w_text, 'start': start, 'end': end, 'confidence': 0.95})
                if w.get('is_repeated'):
                    repeated_words.append({'word': w_text, 'start': start, 'end': end, 'confidence': 0.95})

            # Cache derived arrays so subsequent calls are fast and consistent
            try:
                video_service.videos.update_one(
                    {'_id': ObjectId(video_id)},
                    {'$set': {
                        'transcript.filler_words': filler_words,
                        'transcript.repeated_words': repeated_words,
                    }}
                )
            except Exception as cache_err:
                logger.warning(f"[FILLER DETECTION] Could not cache derived filler arrays: {cache_err}")
...
def get_transcript(user_id, video_id):
    ...
    try:
        ...
        def _transcript_looks_corrupted(transcript_obj):
            words = (transcript_obj or {}).get('words', []) or []
            ...
            long_token_count = sum(
                1
                for w in words
                if len(str((w or {}).get('text', '')).strip()) >= 25
            )
            filler_count = int((transcript_obj or {}).get('filler_count', 0) or 0)
            repeated_count = int((transcript_obj or {}).get('repeated_count', 0) or 0)
            marked_ratio = (filler_count + repeated_count) / max(total_words, 1)

            # Heuristic: transcript likely drifted/hallucinated and should be regenerated.
            return (total_words >= 20 and long_token_count / total_words > 0.15) or (total_words >= 30 and marked_ratio > 0.80)
        ...
        transcript_missing = not video.transcript or video.transcript.get('total_words', 0) == 0
        transcript_corrupted = False if transcript_missing else _transcript_looks_corrupted(video.transcript)

        if transcript_missing or transcript_corrupted:
            # Transcript not generated yet or empty, try to generate now
            reason = 'missing/empty' if transcript_missing else 'quality-check failed'
            logger.info(f"[TRANSCRIPT] Regenerating transcript ({reason})...")
            video_service._auto_transcribe_video(video_id, video.filepath)
...
def download_video_thumbnail(user_id, video_id):
    try:
        ...
        # Optional thumbnail index for downloading a specific generated thumbnail.
        thumbnail_index = request.args.get('index', type=int)
        thumbnails = video.outputs.get('thumbnails', [])
        ...
        if thumbnail_index is not None:
            ...
            thumbnail_path = video.outputs.get('thumbnail') or (thumbnails[0] if thumbnails else None)

        if not thumbnail_path:
            return jsonify({'error': 'Thumbnail not found'}), 404

        # Resolve relative paths so downloads work regardless of launch directory.
...
def delete_user_by_admin(user_id):
    """Delete a user"""
    try:
        # reason is optional; allow DELETE requests without JSON body/content-type
...
@app.route('/api/admin/videos/<video_id>/download', methods=['GET'])
@admin_required
@permission_required('manage_videos')
def download_video_by_admin(video_id):
    """Download processed/original video as admin"""
    try:
        video = video_service.get_video(video_id)
        if not video:
            return jsonify({'error': 'Video not found'}), 404
        ...
        if not processed_path:
            return jsonify({'error': 'Video file path not available'}), 404

        if not os.path.isabs(processed_path):
            processed_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), processed_path)

        if not os.path.exists(processed_path):
            logger.error(f"[ADMIN DOWNLOAD] File not found: {processed_path}")
            return jsonify({'error': 'Video file not found'}), 404

        download_name = f"admin_{video.filename}" if video.filename else f"admin_video_{video_id}.mp4"
        ...
        logger.error(f"Admin download video error: {e}")
